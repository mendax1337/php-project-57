...
@section('content')
    <div class="max-w-3xl mx-auto px-4 py-8">
        <h1 class="text-3xl font-semibold mb-6">Изменить метку</h1>
        ...
        <form method="POST" action="{{ route('labels.update', $label) }}" class="space-y-6 bg-white p-6 rounded-xl shadow">
            @csrf
            @method('PATCH')

            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Имя</label>
                <input id="name" name="name" type="text" value="{{ old('name', $label->name) }}" required
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="block w-full rounded-xl border border-gray-300 px-4 py-2.5 shadow-sm
                             focus:border-blue-500 focus:ring-2 focus:ring-blue-500">{{ old('description', $label->description) }}</textarea>
            </div>
